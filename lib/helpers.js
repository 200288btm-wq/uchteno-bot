...
export const notifyMenu = (settings) => ({
  reply_markup: {
    inline_keyboard: [
      [{ text: settings.notify_before_hours > 0 ? '📅 Напоминания о занятиях: ВКЛ' : '📅 Напоминания о занятиях: ВЫКЛ', callback_data: 'toggle_reminders' }],
    ]
  }
})
...
export async function getClientByTelegram(studioId, telegramId) {
  ...
  const row = rows?.[0]
  if (!row) return null
  console.log('client_telegram row:', JSON.stringify(row))
  const clients = await sbGet('clients', `id=eq.${row.client_id}&limit=1`)
  console.log('clients result:', JSON.stringify(clients))
...
export async function findClientByPhone(studioId, phone) {
  ...
  console.log('findClientByPhone digits:', digits)
  if (!digits || digits.length < 9) return null
  const clients = await sbGet('clients', `studio_id=eq.${studioId}`)
  console.log('clients count:', clients?.length, 'contacts sample:', JSON.stringify(clients?.[0]?.contacts))
