...
export async function getStudioByToken(token) {
  try {
    const url = `${SUPABASE_URL}/rest/v1/studio_settings?bot_token=eq.${encodeURIComponent(token)}&select=*,studios(id,name)&limit=1`
    ...
    console.log('Fetch result:', JSON.stringify(json).slice(0, 100))
    return json?.[0] || null
  } catch (e) {
    console.error('getStudioByToken error:', e.message)
    return null
  }
}
